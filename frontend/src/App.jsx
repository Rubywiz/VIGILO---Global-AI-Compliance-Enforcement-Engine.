...
function Dashboard() {
  ...
  const connectWebSocket = useCallback((sid) => {
    ...
    // INFRASTRUCTURE FIX: Explicitly routing websocket frames through the secure ngrok tunnel
    const wsBase = "wss://untrading-irrelevantly-dustin.ngrok-free.dev";
    const socket = new WebSocket(`${wsBase}/ws/${sid}`, ["ngrok-skip-browser-warning"]);
    
    socket.onopen = () => {
      console.log("🚀 [WebSocket Connected]: Streaming pipeline established over ngrok.");
    };
